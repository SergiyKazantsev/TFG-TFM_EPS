...
Las aplicaciones de dispositivos móviles se han convertido en una parte importante y esencial en nuestras vidas cotidianas, alcanzando casi el 96\% de uso entre la población joven de los países desarrollados \citep{atske-2025}. Este fenómeno ha provocado un auge tanto de las tecnologías nativas del móvil, como las de multiplataforma, alcanzando así una mayor cuota del mercado.
\par Este trabajo tiene como objetivo estudiar los frameworks multiplataforma y desarrollar una aplicación compatible con los navegadores Web y el sistema operativo Android.
...
%A continuación, se ofrece una copia de la información quFe hay en el libro de estilo para la realización de los \gls{tfg} de la EPS de la Universidad de Alicante.
%
...
\par Por otro lado, en España también existen docenas de conservatorios y escuelas musicales que tienen un programa de enseñanza más amplio y regulado por el Gobierno de España y por la Comunidad Autónoma. Dichas instituciones de enseñanza musical ofrecen varios niveles de profesionalización, siendo los estudios superiores de música el nivel más alto.
\par Este \gls{tfg} está destinado a las personas estudiantes de música que poseen conocimientos teóricos elementales y profesionales sobre la lectura y reconocimiento de las partituras y del lenguaje musical, que se alcanza durante la realización de estudios entre las edades de 8 a 18 años. Según el \citet{rd-1577-2006}, los alumnos deben tener, entre otras, las siguientes habilidades:
\begin{itemize}
...
\par Los elementos esenciales del sonido son:
\begin{itemize}
	\item La altura: definida con un nombre (Do, Re; A, B)
	\item La intensidad
	\item La duración
\end{itemize}
\par También las estructuras como:
\begin{itemize}
...
\subsection{Octavas}
El piano dispone de 88 teclas, cada una de las cuales tiene una frecuencia determinada (desde 20 \gls{hz} y hasta 20000 \gls{hz}). La distancia mínima en frecuencia/altura de cada tecla siempre es la misma, y se denomina "semitono". Gracias a los tonos y semitonos, se puede saber exactamente la distancia entre cada nota, por ejemplo: entre Do y Re la distancia es de un tono, entre Do y Do\musSharp{} la distancia es de un semitono (ver figura \ref{fig:octavas}). Matemáticamente, un semitono de distancia corresponde a la multiplicación de una determinada frecuencia por $2^{n/12}$, donde $n$ es el número de semitonos que se quiere añadir a la frecuencia, y 12 corresponde a los 12 semitonos de una octava. 

Por lo cual, dada una frecuencia $h$:

\begin{equation}
	h_{\text{+1 semitono}} = h \cdot 2^{1/12} \approx h \cdot 1.06
\end{equation}

\begin{equation}
	h_{\text{+1 octava}} = h \cdot 2^{12/12} = h \cdot 2
\end{equation}

\begin{figure}[h]
	\centering
	\includegraphics[width=0.95\textwidth]{archivos/octavas}
	\caption{Las octavas del piano \citep{octavas}}
	\label{fig:octavas}
\end{figure}
\newpage
\subsubsection{Intervalos}
Un intervalo es la diferencia entre dos sonidos aislados, y se puede referir tanto a los acordes (suenan simultáneamente) como a las melodías (sonidos sucesivos).
...
\par Cada denominación de cada escala se compone de dos elementos:
\begin{itemize}
...
\par En música existen dos calidades emocionales: la Mayor y la Menor. La calidad mayor proporciona una apariencia "feliz" y "alegre" de la obra, mientras que el modo menor la hace "triste" y "melancólica".
\subsection{Armonía}
La armonía actúa como el soporte de la melodía que sirve para reforzarla y proporcionarle variedad. La misma melodía con distino acompañamiento armónico puede transmitir sensaciones muy diferentes \citep{armonia-musical-def-historia}.
Desde la perspectiva de la teoría musical, la armonía es una combinación de notas (alturas) que suenan al mismo tiempo. Por otro lado, desde el punto de vista práctico, se puede considerar la armonía como la consonancia de la melodía, cuando la melodía es agradable para el oyente.
\subsubsection{Tonalidad}
La tonalidad es una relación organizada de varias notas alrededor de una tónica \citep{alma9911276603502466}. Esto significa que cada fragmento de una composición musical se "inclina" hacia dicha tónica, se construye alrededor de ella, y normalmente se empieza y termina la composición con ella.
\par El concepto de la tonalidad está muy relacionado con la modalidad:
\begin{figure}[H]
	\centering
	\includegraphics[width=14cm]{archivos/circulo}
	\caption{El círculo de quintas \citep{circulo_quintas-2008}}
	\label{fig:circulo}
\end{figure}
\par La figura visualiza la relación entre la modalidad (Mayor o Menor) y la tonalidad (Do, Re, etc.). Cada tonalidad del diagrama corresponde a uno de los doce semitonos de la escala crómatica. El nombre del círculo proviene del hecho de que cada tonalidad está a 3.5 semitonos de distancia (intervalo de quinta) a cada uno de sus tonalidades adyacentes. Es utilizado por los músicos para componer y armonizar sus composiciones.
\subsubsection{Acordes}
El acorde es una composición de tres o más notas que suenan a la vez. Una combinación determinada de notas en un acorde 
se crea haciendo uso del "circulo de terceras", parecido al de circulo de quintas, que se aplica a las notas y no a los semitonos.
\begin{figure}[H]
	\centering
	\includegraphics[width=10cm]{archivos/terceras}
	\caption{Un acorde tonal tríada (izquierda) en comparacion con un acorde atonal \citep{terceras}}
	\label{fig:terceras}
\end{figure}
\subsection{Ritmo}
Las composiciónes musicales occidentales están formados por fragmentos de una duración determinada, lo que le proporciona el ritmo y pulos. La longitud de cada fragmento es definida por el compás: \musFig{4 4}, \meterC, \musFig{2 2}, \meterCutC, etc.
\par El numero de figuras musicales dentro de un fragmento se calcula con el compás y la duracion relativa de la figura, lo que se denomina el ritmo:
\begin{table}[ht]
	\centering
	{\scalefont{0.9}
		\begin{tabular}{@{}ccl@{}}
			\toprule
			Figura & Silencio & Duración	\\ \midrule
			\musWhole & \musSymbol{0.955em}{0.55ex}{0pt}{\symbol{10}} & 4 tiempos       	\\
			\musHalf & \musSymbol{0.955em}{0.55ex}{0pt}{\symbol{11}}	& 2 tiempos      			\\
			\musQuarter & \musSymbol{0.955em}{0.55ex}{0pt}{\symbol{62}}	& 1 tiempo   \\
			\musEighth &  \musSymbol{0.955em}{0.55ex}{0pt}{\symbol{63}}	& 1/2 tiempo      			\\
			\musSixteenth & \musSymbol{0.955em}{0.55ex}{0pt}{\symbol{64}}	& 1/4 tiempo	\\
			\musThirtySecond & \musSymbol{0.955em}{0.55ex}{0pt}{\symbol{65}}	& 1/8 tiempo	\\
			\musSixtyFourth	& \musSymbol{0.955em}{0.55ex}{0pt}{\symbol{66}}& 1/16 tiempo
			\\	 
			\bottomrule
		\end{tabular}
	}
	\caption{Las duraciones de distintas figuras para el compás \musFig{4 4}, el cual será utilizado en este \gls{tfg}.}
	\label{tablaritmo}
\end{table}
...
Actualmente, la forma más eficiente del aprendizaje de la música, y la preferida por los tutores, es la asistencia a los conservatorios y escuelas de música de forma extracurricular. En consecuencia, el niño es obligado a asistir tanto a su escuela de primaria/secundaria como a la escuela de música. Además, está obligado a la realización de las tareas académicas de ambas durante su tiempo libre, lo cual podría generar falta de motivación, resentimiento y el síndrome de agotamiento (\textit{burnout}). Por otro lado, los alumnos también informan de la falta de reconocimiento y la falta de apoyo \citep{orzel-2010}.
...
La digitalización de los procesos educativos ha tenido un mayor avance durante las últimas décadas: cursos digitales, profesores, enseñanza a distancia, etc. Dentro de este proyecto, se ofrece una solución tecnológica a la cuestión de la ausencia de instrumentos y recursos en el estudiantado joven, para proporcionar un producto que permita el entrenamiento el oído sin usar el piano físico, facilitando así los procesos de la memorización y puesta en práctica de los conceptos teóricos. En las siguientes secciones, se definirá el alcance del proyecto con el estudio previo de las tecnologías de aprendizaje de música ya existentes.

\section{Objetivos}
\label{objetivos}
Esta sección expone los objetivos principales de este proyecto, así como los objetivos secundarios. Todos los objetivos tendrán que ser medibles, alcanzables y específicos dentro del contexto del proyecto, y la definición de los objetivos permitirá desglosar la planificación del proyecto en tareas concretas una vez que se haya empezado la implementación.
\par El objetivo principal de este trabajo es desarrollar una aplicación interactiva multiplataforma para el entrenamiento de oído musical, que permitirá a los estudiantes de las escuelas de música a reforzar y poner en práctica los conceptos aprendidos a lo largo del horario lectivo.
\par Los objetivos secundarios serán:
\begin{itemize}
	\item La aplicación será multiplataforma, disponible tanto desde el navegador como desde la aplicación de Android.
	\item Diseñar una aplicación con interfaz interactiva y agradable, que sea fácil de usar incluso para los usuarios jóvenes (<10 años). Para ello, se aplicarán los principios del buen diseño de \gls{ui} y \gls{ux}.
	\item Generar los audios para cada una de las lecciones.
	\item Incluir grabaciones de audio con el fin de proporcionar la base para el entrenamiento.
	\item Cada estudiante podrá registrarse, iniciar sesión, acceder a su perfil y visualizar y medir su progreso dentro de la aplicación.
	\item Las lecciones de aprendizaje ordenadas de forma progresiva según su dificultad, y se podrán abrir una vez terminada la lección previa. Por ejemplo, el entrenamiento del reconocimiento de escalas implica un buen manejo de reconocimiento de los intervalos.
	\item Las lecciones tendrán que ser variadas según el tipo del entrenamiento.
	\item Publicar la aplicación en un hosting.
\end{itemize}
\par Por otro lado, se tendrá que incluir (aunque no se limitará a ello) el entrenamiento de:
\begin{enumerate}
	\item Reconocimiento de diferencia de altura entre notas (cuál es la más alta/más baja).
	\item Reconocimiento de tonos y semitonos.
	\item Reconocimiento de intervalos.
	\item Reconocimiento de escalas.
	\item Reconocimiento de acordes.
	\item Reconocimiento de ritmos.
\end{enumerate}
